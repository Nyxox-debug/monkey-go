...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefixFns(token.INT, p.parseIntegerLiteral)
	...
	p.registerPrefixFns(token.FUNCTION, p.parsePrefixFunctionLiteral)

	p.infixParseFns = make(map[token.TokenType]infixParseFn)
	p.registerInfixFns(token.PLUS, p.parseInfixExpression)
	p.registerInfixFns(token.MINUS, p.parseInfixExpression)
	p.registerInfixFns(token.SLASH, p.parseInfixExpression)
	...
	p.registerInfixFns(token.LT, p.parseInfixExpression)
	p.registerInfixFns(token.GT, p.parseInfixExpression)
...
func (p *Parser) ParseProgram() *ast.Program {
	// NOTE: Creating the root node of the AST
	program := &ast.Program{}
...
// NOTE: This is an assertion function
...
func (p *Parser) registerPrefixFns(tokenType token.TokenType, fn prefixParseFn) {
	p.prefixParseFns[tokenType] = fn
}

func (p *Parser) registerInfixFns(tokenType token.TokenType, fn infixParseFn) {
...
// TODO: Read this again and again and again and again and again and again and again
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	// NOTE: leftExp does'nt mean LEFT Exp for an operator but the remaining Exprssion ie the expression left
	leftExp := prefix()
